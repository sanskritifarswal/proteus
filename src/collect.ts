...
export function validateRecord(rec: unknown): string[] {
  ...
  r.events.forEach((e: Partial<UIEvent>, i) => {
    ...
    const art = 'article' in e && (e as { article?: unknown }).article !== undefined;
    const val = 'value' in e && (e as { value?: unknown }).value !== undefined;
    switch (e.type) {
      ...
      case 'dwell': case 'complete':
        ...
        if (!val || !Number.isFinite((e as { value: number }).value)) errors.push(`${at} ${e.type} needs a numeric value`);
        else if (e.type === 'complete' && ((e as { value: number }).value < 0 || (e as { value: number }).value > 1)) errors.push(`${at} complete value must be in 0..1`);
        break;
      case 'action': if (typeof (e as { action?: unknown }).action !== 'string') errors.push(`${at} action needs action`); break;
...
/** Group by user, order by session, derive `returned` from the next session's existence (censored on the last). */
...
if (process.argv[1] && process.argv[1].endsWith('collect.ts')) {
  ...
  for (const [user, sessions] of assemble(records)) {
