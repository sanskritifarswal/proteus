...
console.log(`twins experiment: ${iterations} iterations x ${users} users, seeds ${seeds.join(',')}; held-out 400 twins\n`);
...
for (const seed of seeds) {
  for (const [label, mask] of [['full', []], ['ablated', EVIDENCE]] as Array<[string, number[]]>) {
    const { policy } = train({ iterations, usersPerIteration: users, maxSessions, lr: 0.02, l2: 0.001, seed, archetypes: TWINS, maskFeatures: mask });
